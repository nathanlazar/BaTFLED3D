...
plot_test_RMSE <- function(test.results, ylim='default', main='Test RMSEs',
  ...
  if(ylim=='default') {
    ylim <- c(0, 1.5)
  } else if(is.na(ylim)) ylim=c(0,max(sub.results, na.rm=T))

  rmse.cols <- grep('RMSE$', names(sub.results))
  types <- sub('.RMSE', '', names(sub.results)[rmse.cols])
  ...
  for(i in 1:n) {
    points(sub.results[,rmse.cols[i]], col=colrs[i], pch=20, type='b')
  }

  for(i in 1:n)
    abline(h=baselines[i], col=colrs[i], lty=2, lwd=2)
