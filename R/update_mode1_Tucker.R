...
update_mode1_Tucker <- function(m, d, params, batch.samps) {
  ...
  rho <- (m$iter + 1)^(-kappa)
  I <- batch[1]; J <- batch[2]; K <- batch[3]
    
  all_I <- dim(d$resp)[1]; all_J <- dim(d$resp)[2]; all_K <- dim(d$resp)[3]
  ...
  mode2.H.mean <- m$mode2.H.mean[batch.samps[[2]],]
  mode2.H.var <- m$mode2.H.var[batch.samps[[2]],]
  mode3.H.mean <- m$mode3.H.mean[batch.samps[[3]],]
  mode3.H.var <- m$mode3.H.var[batch.samps[[3]],]
  sigma2 <- m$sigma2
  m1.sigma2 <- m$m1.sigma2
  batch.delta <- d$delta[batch.samps[[1]], batch.samps[[2]], batch.samps[[3]]]
    
  if(H1.intercept) {
    m$mode1.H.var[batch.samps[[1]],-1] <- foreach(
      delta=iterators::iapply(batch.delta, 1), .combine='rbind') %:%
    ...
    m$mode1.H.var[batch.samps[[1]],] <- foreach(delta=iterators::iapply(batch.delta, 1), .combine='rbind') %:%
  ...
  } else x_times_a <- safe_prod(d$mode1.X[batch.samps[[1]],], m$mode1.A.mean)
  if(H1.intercept) x_times_a <- cbind(1, x_times_a)
  sum0 <- rTensor::ttl(rTensor::as.tensor(m$core.mean), 
                       list(m$mode2.H.mean[batch.samps[[2]],], 
                            m$mode3.H.mean[batch.samps[[3]],]), c(2,3))@data
  
  # Update the mean parameters (m$mode1.H.mean)
  core.mean <- m$core.mean
  batch.m1.H.mean <- m$mode1.H.mean[batch.samps[[1]],]
  batch.m1.H.var <- m$mode1.H.var[batch.samps[[1]],]
  batch.resp <- d$resp[batch.samps[[1]], batch.samps[[2]], batch.samps[[3]]]
  ...
  m$mode1.H.mean[batch.samps[[1]],] <- foreach(mode1.H.mean = iterators::iter(batch.m1.H.mean, by='row'), 
                            mode1.H.var = iterators::iter(batch.m1.H.var, by='row'),
                            resp = iterators::iapply(batch.resp, 1),
  ...
  ## HERE ## Update A matrices.
  
  # If the intercept term is removed change A1.intercept  
  A1.intercept <- ifelse('const' %in% rownames(m$mode1.A.mean), T, F)

  # Get intermediate estimates of A for each sample in the batch
  
  if(P != 0) { # If there is no input data, skip updates for A
