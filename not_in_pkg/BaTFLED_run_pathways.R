...
# CTRP2 dataset
# dir <- 'D:/Box Sync/Thesis/'
# args <- list(paste0(dir, 'CTRP2/Rem_NAs_COSMIC/CTRP2_cl_train_gr.Rdata'),
#              paste0(dir, 'CTRP2/Removed_NAs/dr_train_mat.Rdata'),
#              'none',
#              paste0(dir, 'CTRP2/Removed_NAs/raw_train_resp.Rdata'),
#              paste0(dir, 'CTRP2/Removed_NAs_10fold_cv/cv_folds_cl.Rdata'),
#              paste0(dir, 'CTRP2/Removed_NAs_10fold_cv/cv_folds_dr.Rdata'),
#              'CTRP2results/test/',
#              'pathways=D:/Box Sync/Thesis/Pathways/kegg_cancer.Rdata',
#              'decomp=Tucker',
#              'row.share=F',
#              'reps=30',
#              'warm.per=0.05',
...
# Get pathway file if it exists
if(sum(grepl('pathways', args))) {
  path.file <- sub('pathways=', '', args[grepl('pathways', args)][[1]])
}
...
if(exists('path.file')) paths <- loadRData(path.file)
...
# If pathways are provided, change the priors
print('Pathways associated with latent factors for mode 1')
genes <- sapply(rownames(model$mode1.lambda.shape), function(x) strsplit(x, split='.', fixed=T)[[1]][1])
for(path in 1:min(nrow(paths), ncol(model$mode1.lambda.shape))) {
  p.genes <- strsplit(paths$entrez_gene_ids[path], split=',')[[1]]
  model$mode1.lambda.shape[genes %in% p.genes, path] <- 1
  model$mode1.lambda.scale[genes %in% p.genes, path] <- 1
  # Print pathway associations
  print(paths$pathway[path])
}
...
for(i in (trained$iter+1):reps) {
  ...
  test.results <- test_results(m=trained, d=train.data, test.results=test.results,
                               ...
                               test.m2=test.data.m2, test.m1m2=test.data.m1m2)
