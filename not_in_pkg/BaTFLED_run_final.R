...
# Usage: BaTFLED_run_final.R <mode1 data> <mode2 data> <mode3 data>
#                            <response> 
#                            <output dir>
#                            <options...>

# The first 5 arguments must be in order
...
# Read in options               #### TODO: add cv.folds for mode 3 ####
m1.file <-        args[[1]]
m2.file <-        args[[2]]
m3.file <-        args[[3]]
resp.file <-      args[[4]]
out.dir <-        paste0(args[[5]], '/')

# Get parameters
params <- get_model_params(args[6:length(args)])

# Set some defaults 
reps <- 10; warm.per <- 0.05
multiplier <- 0; quantile=F; center=F; 
scale='no'; squash=Inf; history=F; fold=1
...
m1.mat <- m1.mat[,(apply(m1.mat, 2, sd) !=0) & !is.na(apply(m1.mat, 2, sd))]
m2.mat <- m2.mat[,(apply(m2.mat, 2, sd) !=0) & !is.na(apply(m2.mat, 2, sd))]
m3.mat <- m3.mat[,(apply(m3.mat, 2, sd) !=0) & !is.na(apply(m3.mat, 2, sd))]
...
if(center) {
  ...
  m1.center <- rep(NA, ncol(m1.mat))
  names(m1.center) <- dimnames(m1.mat)[[2]]
  m1.center[!m1.bin] <- apply(m1.mat[,!m1.bin], 2, mean)
  m1.mat[,!m1.bin] <- scale(m1.mat[,!m1.bin], scale=F, center=T)
  m2.center <- rep(NA, ncol(m2.mat))
  names(m2.center) <- dimnames(m2.mat)[[2]]
  m2.center[!m2.bin] <- apply(m2.mat[,!m2.bin], 2, mean)
  m2.mat[,!m2.bin] <- scale(m2.mat[,!m2.bin], scale=F, center=T)
  m3.center <- rep(NA, ncol(m3.mat))
  names(m3.center) <- dimnames(m3.mat)[[2]]
  m3.center[!m3.bin] <- apply(m3.mat[,!m3.bin], 2, mean)
...
if(scale=='all') {
  ...
  m1.scale <- attr(m1.mat, 'scaled:scale')
  m2.mat <- scale(m2.mat, center=F, scale=apply(m2.mat, 2, sd, na.rm=T))
  m2.scale <- attr(m2.mat, 'scaled:scale')
  m3.mat <- scale(m3.mat, center=F, scale=apply(m3.mat, 2, sd, na.rm=T))
  m3.scale <- attr(m3.mat, 'scaled:scale')
  } else if(scale=='cont') {
    # Scale only continuous (non-binary variables)
    m1.scale <- rep(NA, ncol(m1.mat))
    names(m1.scale) <- dimnames(m1.mat)[[2]]
    m1.scale[!m1.bin] <- apply(m1.mat[,!m1.bin], 2, sd)
    m1.mat[,!m1.bin] <- scale(m1.mat[,!m1.bin], center=F, 
                              scale=apply(m1.mat[,!m1.bin], 2, sd, na.rm=T))
    m2.scale <- rep(NA, ncol(m2.mat))
    names(m2.scale) <- dimnames(m2.mat)[[2]]
    m2.scale[!m2.bin] <- apply(m2.mat[,!m2.bin], 2, sd)
    m2.mat[,!m2.bin] <- scale(m2.mat[,!m2.bin], center=F, 
                              scale=apply(m2.mat[,!m2.bin], 2, sd, na.rm=T))
    m3.scale <- rep(NA, ncol(m3.mat))
    names(m3.scale) <- dimnames(m3.mat)[[2]]
    m3.scale[!m3.bin] <- apply(m3.mat[,!m3.bin], 2, sd)
...
print(sprintf('Cold start predictions will be made using %d mode 1 predictors, %d mode 2 and %d mode3 predictors.',
