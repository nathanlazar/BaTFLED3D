...
newer.args <- commandArgs(TRUE)
test <- F
if(test)
  newer.args <- c('HEISERresults/run_2409514.0/all.Rdata')
...
if(exists('test.m1') && length(test.m1)) {
...
if(exists('test.m2') && length(test.m2)) {
...
if(exists('test.m1') && exists('test.m2') && length(test.m1) && length(test.m2)) {
...
for(i in 1:I) mean.tens.list$train.m1[i,,] <- m2.means
for(j in 1:J) mean.tens.list$train.m2[,j,] <- m1.means
